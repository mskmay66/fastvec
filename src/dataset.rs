...
#[pyfunction]
pub fn build(documents: Vec<Vec<String>>, vocabulary: Vocab, window: mut Option<usize>) -> PyResult<Vec<(usize, usize, u8)>> {
    let mut examples = Vec::new();
    documents.par_iter().for_each(|doc| {
        let encoded_doc: Vec<usize> = vocabulary.get_ids(&doc);
        window = window.unwrap_or(5) as u32; // Default context window size if not provided
        for w in encoded_doc.windows(window) { // Assuming context window size of 5
